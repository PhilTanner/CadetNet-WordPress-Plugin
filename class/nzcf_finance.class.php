<?php
	...
	class NZCF_Finance extends NZCF
	{
		public function add_payment( $personnel_id, $amount, $reference, $payment_type, $related_to_id )
		{
			if(!self::user_has_permission( NZCF_PERMISSION_FINANCE_EDIT ))
		...
		function currency_format( $format, $amount )
		{
			...
			switch($format)
			{
				case NZCF_SETTING_MONEYFORMAT_PARENTHESIS:
					if( (float)$amount < 0 )
						$str .= '(';
					$str .= '$ ';
					...
					if( (float)$amount < 0 )
						$str .= ')';
					break;
				case NZCF_SETTING_MONEYFORMAT_TEXTUAL:
					$str .= '$ ';
		...
		{
			if( !self::user_has_permission(NZCF_PERMISSION_FINANCE_VIEW, $personnel_id) )
			    throw new NZCFExceptionInsufficientPermissions("Insufficient rights to view this page");
			...
			$query = '
				SELECT
					...
					`payment`.`reference`,
					CASE 
						WHEN `payment_type` < '.NZCF_PAYMENT_TYPE_RECEIPT_TERM_FEE.' 
		...
		{
			if(!self::user_has_permission( NZCF_PERMISSION_ACTIVITIES_VIEW ))
			    throw new NZCFExceptionInsufficientPermissions("Insufficient rights to view this page");
			if( !self::user_has_permission(NZCF_PERMISSION_FINANCE_VIEW, $personnel_id) )
			    throw new NZCFExceptionInsufficientPermissions("Insufficient rights to view this page");
			...
			$query = '
				...
				FROM 
					...
					LEFT JOIN (
						SELECT 
							...
							CASE WHEN `payment_type` = '.NZCF_PAYMENT_TYPE_INVOICE_ACTIVITY_FEE.' THEN `payment`.`amount` ELSE 0 END AS `amount_due`,
							CASE WHEN `payment_type` = '.NZCF_PAYMENT_TYPE_RECEIPT_ACTIVITY_FEE.' THEN (0-`payment`.`amount`) ELSE 0 END AS `amount_paid`,
						...
						WHERE 
							`payment_type` IN ('.NZCF_PAYMENT_TYPE_INVOICE_ACTIVITY_FEE.','.NZCF_PAYMENT_TYPE_RECEIPT_ACTIVITY_FEE.')
					) `payments_tmp`
				...
				WHERE 
					...
					AND NOT `personnel`.`access_rights` IN ('.NZCF_USER_GROUP_OFFICERS.')
				GROUP BY 
					`payments_tmp`.`activity_id`, 
					`personnel`.`personnel_id`
			...
			$query = '
				...
				FROM 
					...
					INNER JOIN `term`
						...
						AND `attendance_register`.`presence` IN ( '.NZCF_ATTENDANCE_PRESENT.','.NZCF_ATTENDANCE_ABSENT_WITHOUT_LEAVE.' )
					LEFT JOIN `payment`
						ON `payment`.`related_to_id` = `term`.`term_id` 
						AND `payment`.`personnel_id` = `personnel`.`personnel_id` 
						AND `payment`.`payment_type` = '.NZCF_PAYMENT_TYPE_INVOICE_TERM_FEE.'
				WHERE
			...
			$query = '
				...
				FROM 
					...
					LEFT JOIN (
						SELECT 
							`payment`.`personnel_id`,
							CASE WHEN `payment_type` = '.NZCF_PAYMENT_TYPE_INVOICE_TERM_FEE.' THEN `payment`.`amount` ELSE 0 END AS `amount_due`,
							CASE WHEN `payment_type` = '.NZCF_PAYMENT_TYPE_RECEIPT_TERM_FEE.' THEN (0-`payment`.`amount`) ELSE 0 END AS `amount_paid`,
							`term`.`startdate`,
							`term`.`enddate`,
							`term`.`term_id`
						FROM 
							...
							INNER JOIN `term`
								ON `payment`.`related_to_id` = `term`.`term_id`
						WHERE 
							`payment_type` IN ('.NZCF_PAYMENT_TYPE_INVOICE_TERM_FEE.','.NZCF_PAYMENT_TYPE_RECEIPT_TERM_FEE.')
					) `payments_tmp`
						ON `payments_tmp`.`personnel_id` = `personnel`.`personnel_id`
				WHERE 
					`personnel`.`enabled` = -1
