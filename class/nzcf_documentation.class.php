<?php
	...
	class NZCF_Documentation extends NZCF
	{
		...
		public function dump_userperms()
		{
			...
			foreach( $constants as $constant => $val )	
				if( substr($constant, 0, strlen('NZCF_PERMISSION_')) == 'NZCF_PERMISSION_' )
					echo '<th>'.str_replace("_", " ", strtolower(substr($constant, strlen('NZCF_PERMISSION_'), 100))).'</th>';
			echo '		</tr>';
			echo '	</thead>';
			echo '	<tbody>';
			foreach( $constants as $constant => $val )
				if( substr($constant, 0, strlen('NZCF_USER_LEVEL_')) == 'NZCF_USER_LEVEL_' )
				{
					echo '<tr>';
					echo '	<th>'.str_replace("_", " ", strtolower(substr($constant, strlen('NZCF_USER_LEVEL_'), 100))).'</th>';
					echo '	<td>'.$val.'</td>';
					foreach( $constants as $perm => $value )
						if( substr($perm, 0, strlen('NZCF_PERMISSION_')) == 'NZCF_PERMISSION_' )
						{
							echo '<td>';
		...
		{
			if(!self::user_has_permission( NZCF_PERMISSION_ATTENDANCE_VIEW ) )
		...
		public function get_cadet_attendance( $startdate, $enddate )
		{
			...
			$query = '
				...
				WHERE 	`attendance_register`.`date` BETWEEN "'.date('Y-m-d', $startdate).'" AND "'.date('Y-m-d', $enddate).'"
					AND `attendance_register`.`presence` = '.NZCF_ATTENDANCE_PRESENT.'
		...
		{
			if(!self::user_has_permission( NZCF_PERMISSION_PERSONNEL_VIEW) )
			    throw new NZCFExceptionInsufficientPermissions("Insufficient rights to view this page");
			...
			$query = '
				...
				WHERE	`personnel`.`access_rights` IN ( '.NZCF_USER_GROUP_CADETS.' )
		...
		public function get_officer_attendance( $startdate, $enddate, $supplimentary=false )
		{
			...
			$query = '
				SELECT	`personnel`.`personnel_id`,
					...
					(
						...
						WHERE `activity_register`.`personnel_id` = `personnel`.`personnel_id`
							AND `activity_register`.`presence` = '.NZCF_ATTENDANCE_PRESENT.'
				...
				WHERE 	`attendance_register`.`date` BETWEEN "'.date('Y-m-d', $startdate).'" AND "'.date('Y-m-d', $enddate).'"
					AND `attendance_register`.`presence` = '.NZCF_ATTENDANCE_PRESENT.'
					AND `personnel`.`access_rights` IN ('.NZCF_USER_GROUP_OFFICERS.','.NZCF_USER_LEVEL_SNCO.')
